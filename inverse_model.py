...
class Encoder(nn.Module):
    def __init__(self,im_wh=(64,64),in_ch=3,h_ch=32,embed_len=32, batch_norm=False):
...
class InverseModel(nn.Module):
    def __init__(self,in_ch=3,im_wh=(64,64), h_ch=32, num_actions=3, embed_len=32, batch_norm=False):
        super(InverseModel,self).__init__()
        self.enc = Encoder(im_wh = im_wh, 
                           in_ch=in_ch,
                           h_ch=h_ch,
                           embed_len=embed_len,
...
class QNet(nn.Module):
    def __init__(self,embed_len=32, num_actions=3, h_ch=32):
        super(QNet,self).__init__()
        self.q_enc = nn.Sequential(
            nn.Linear(in_features=embed_len,out_features=h_ch),
            nn.ELU(),
